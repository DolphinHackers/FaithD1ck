...
public class ModuleFarmHunterESP extends CheatModule {
    ...
    public final Handler<UpdateEvent> updateEventHandler = event -> {
        ...
        if (!FaithD1ck.moduleManager.getModule(ModuleKillAura.class).getState()) {
            FaithD1ck.INSTANCE.getRotationManager().setRotation(new Vector2f(mc.thePlayer.rotationYaw, 0), 180f, false, false);
        }
    };
            ...
            if (packet instanceof S13PacketDestroyEntities) {
                for (int entityID : ((S13PacketDestroyEntities) packet).getEntityIDs()) {
                    try {
                        ...
                        if (entity instanceof EntityPlayer && mc.thePlayer.getDistanceSq(new BlockPos(entity.posX, entity.posY, entity.posZ)) <= 40) {
                            FaithD1ck.notificationManager.pop("FarmHunterESP", "A player despawn.", NotificationType.INFO);
                ...
                for (Pair<Long, BlockPos> posPair : playerDespawnPos) {
                    ...
                    if (distance <= 1) {
                        ...
                        FaithD1ck.notificationManager.pop("FarmHunterESP", "Found a similar fake animal", NotificationType.INFO);
                    } else {
                        for (Entity entity : mc.theWorld.loadedEntityList) {
                ...
                if (bpt >= 0.45) {
                    mentities.add(entity);
                    FaithD1ck.notificationManager.pop("FarmHunterESP", "A animal moves too quickly, bps=" + bpt * 20D, NotificationType.INFO);
                }
            }
        }
    };
