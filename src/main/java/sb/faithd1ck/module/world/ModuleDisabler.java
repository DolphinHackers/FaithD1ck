...
public class ModuleDisabler extends CheatModule {
    private ValueMode mode = new ValueMode("Mode", new String[]{"GrimPost","Watchdog","BlocksMC"}, "Watchdog");
    ...
    public static boolean getGrimPost() {
        return mc.thePlayer != null && mc.theWorld != null && FaithD1ck.moduleManager.getModule(ModuleDisabler.class).mode.is("GrimPost") && FaithD1ck.moduleManager.getModule(ModuleDisabler.class).getState() && mc.thePlayer.ticksExisted > 30;
    }
    ...
    private Handler<PacketEvent> packetEventHandler = event -> {
        ...
        if (event.getType() == PacketEvent.Type.SEND) {
            ...
            if(mode.is("Watchdog")){
                if (event.getPacket() instanceof C03PacketPlayer && (!lowhop.getValue() || !disabled)) {
            ...
            if(mode.is("Watchdog") && lowhop.getValue() && disabled){
                ...
                if(event.getPacket() instanceof S08PacketPlayerPosLook) {
                    ...
                    if(testTicks == 20) {
                        ...
                        disabled = false;
                        testTicks = 0;
                        FaithD1ck.notificationManager.pop("Disabler","Disabled Watchdog Motion Checks Successfully", 3000, NotificationType.SUCCESS);
                    }
                    mc.thePlayer.motionY = mc.thePlayer.motionZ = mc.thePlayer.motionX = 0;
                }
            }
    ...
    private Handler<MotionEvent> motionEventHandler = event -> {
        if (lowhop.getValue() && mode.is("Watchdog")) {
            if(mc.thePlayer.onGround && jump) {
                ...
                FaithD1ck.notificationManager.pop("Disabler", "Disabler is working,do not move", 3000, NotificationType.WARNING);
                jump = false;
                disabled = true;
            } else if(disabled && mc.thePlayer.offGroundTicks >= 10) {
                if (!ServerUtils.isHypixel() || ModuleHypixelUtils.getCurrentServer() == Servers.NONE) {
                    FaithD1ck.notificationManager.pop("Disabler", "Skip disabler.", NotificationType.INFO);
                    jump = false;
                    disabled = false;
                    testTicks = 0;
                    return;
                ...
                if(FaithD1ck.moduleManager.getModule(ModuleSpeed.class).getState()) {
                    FaithD1ck.notificationManager.pop("Disabler","Disabled module Speed due to disabler is working", 3000, NotificationType.INFO);
                    FaithD1ck.moduleManager.getModule(ModuleSpeed.class).setState(false);
                }
