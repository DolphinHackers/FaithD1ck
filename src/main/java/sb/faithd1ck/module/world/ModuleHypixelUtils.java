...
import sb.faithd1ck.event.impl.HypixelServerSwitchEvent;
...
import sb.faithd1ck.utils.tasks.FutureTask;
...
public class ModuleHypixelUtils extends CheatModule {
    ...
    private final Handler<HypixelServerSwitchEvent> updateEventHandler = event -> {
        if (event.lastServer == Servers.PRE) {
                            ...
                            for (int z = -20; z < 21; z++) {
                                for (int y = -10; y < 12; y++) {
                                    ...
                                    if (mc.theWorld.getBlockState(pos).getBlock() == Blocks.bed && mc.theWorld.getBlockState(pos).getValue(BlockBed.PART) == BlockBed.EnumPartType.HEAD) {
                                        ...
                                        FaithD1ck.notificationManager.pop("Whitelisted your own bed!", "Whitelisted bed at " + pos, 3000, NotificationType.INFO);
                                    }
                                }
                            }
                        }
            ...
            if (event.server != Servers.NONE) {
                ...
                FaithD1ck.notificationManager.pop("HypixelUtils", "Game Started! Mode: " + event.server.name(), NotificationType.INFO);
            }
        }
        ...
        if (packetEventReceive.getPacket() instanceof S02PacketChat) {
            try {
                ...
                if (command.startsWith("/play ")) {
                    ...
                    FaithD1ck.notificationManager.pop("Sending you to the next game in ", delay.getValue() * 1000, NotificationType.INFO);

                    FaithD1ck.INSTANCE.getTaskManager().queue(new FutureTask(this.delay.getValue() * 1_000) {
    ...
    private void autoscreenshot(PacketEvent e) {
        if (e.getPacket() instanceof S45PacketTitle) {
            ...
            if (packet.getMessage().getUnformattedText().contains("VICTORY!")) {
                FaithD1ck.INSTANCE.getTaskManager().queue(new FutureTask(auto_screenshot_delay.getValue()) {
                    @Override
                    public void execute() {
        ...
        if (event.getType() == PacketEvent.Type.RECEIVE) {
            ...
            if (ServerUtils.isHypixel() && event.getPacket() instanceof S3DPacketDisplayScoreboard) {
                ...
                if (_currentServer != currentServer) {
                    FaithD1ck.INSTANCE.getEventManager().callEvent(new HypixelServerSwitchEvent(currentServer, _currentServer));
