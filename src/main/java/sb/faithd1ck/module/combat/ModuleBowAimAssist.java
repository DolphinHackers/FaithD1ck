...
public class ModuleBowAimAssist extends CheatModule {
    ...
    private final Handler<MotionEvent> motionHandler = event -> {
        ...
        if (silent.getValue()) {
            FaithD1ck.INSTANCE.getRotationManager().setRotation(new Rotation(rotation[0], rotation[1]), 180F, movefix.getValue());
        } else {
    ...
    public boolean isValid(EntityPlayer entity, double range) {
        ...
        return !FaithD1ck.moduleManager.getModule(ModuleAntiBot.class).isBot(entity);
    }
