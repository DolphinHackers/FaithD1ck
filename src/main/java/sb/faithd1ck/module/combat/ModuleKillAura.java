...
public class ModuleKillAura extends CheatModule {
    ...
    private final Handler<UpdateEvent> updateEventHandler = event -> {
        ...
        if (disableOnScaffold.getValue() && FaithD1ck.moduleManager.getModule(ModuleScaffold.class).getState()) {
            target = null;
        }

        if (target != null && mc.thePlayer.getClosestDistanceToEntity(target) <= getRange()) {
            float[] rotation = getRot();
            FaithD1ck.INSTANCE.getRotationManager().setRotation(new Vector2f(rotation[0], rotation[1]), 180f,
    ...
    public static boolean shouldAttack() {
        ...
        if (disableOnScaffold.getValue() && FaithD1ck.moduleManager.getModule(ModuleScaffold.class).getState()) return false;
    ...
    private final Handler<MotionEvent> motionEventHandler1 = event -> {
        ...
        if (event.isPre()) {
            ...
            // preReleaseBlock
            if (targets.isEmpty() && attackMode.is("Pre") && !abMode.is("Interact") && isBlocking) {
                stopBlocking(true);
            }
            ...
            if (abMode.is("Interact") && shouldBlock()) {
                doBlock();
            }
    ...
    void doBlock() {
        if (mc.thePlayer.getHeldItem() != null && mc.thePlayer.getHeldItem().getItem() instanceof ItemSword && !FaithD1ck.moduleManager.getModule(ModuleBlink.class).getState()) {
    ...
    public boolean isValid(Entity entity, double range) {
        ...
        if(FaithD1ck.moduleManager.getModule(ModuleAntiBot.class).isBot(entity))return false;

        return entity instanceof EntityPlayer && targetsvalue.isEnabled("Players") && !entity.getName().contains("LoyisaIs");
    }
