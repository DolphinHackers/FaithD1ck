...
//        }
        ...
        while (!FaithD1ck.moduleManager.getModule(ModuleStuck.class).packets.isEmpty()) {
            mc.getNetHandler().getNetworkManager().sendPacket(FaithD1ck.moduleManager.getModule(ModuleStuck.class).packets.poll(), true);
...
//        return true;
    ...
    public static void onS08() {
        FaithD1ck.moduleManager.getModule(ModuleStuck.class).s08Flag = true;

        FaithD1ck.moduleManager.getModule(ModuleStuck.class).closing = true;
        FaithD1ck.moduleManager.getModule(ModuleStuck.class).setState(false);
        FaithD1ck.moduleManager.getModule(ModuleStuck.class).closing = false;
    }

    public static void throwPearl(Vector2f current) {
        if (!FaithD1ck.moduleManager.getModule(ModuleStuck.class).getState()) return;
        ...
        current.x -= current.x % gcd;
        current.y -= current.y % gcd;
        if (!FaithD1ck.moduleManager.getModule(ModuleStuck.class).rotation.equals(current)) {
            mc.getNetHandler().addToSendQueue(new C03PacketPlayer.C05PacketPlayerLook(current.x, current.y, FaithD1ck.moduleManager.getModule(ModuleStuck.class).onGround), true);
        }
        FaithD1ck.moduleManager.getModule(ModuleStuck.class).rotation = current;
        mc.getNetHandler().addToSendQueue(new C08PacketPlayerBlockPlacement(mc.thePlayer.getHeldItem()), true);
    }
}
