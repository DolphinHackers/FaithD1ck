...
import sb.faithd1ck.module.movement.ModuleGuiMove;
import sb.faithd1ck.module.world.ModuleScaffold;
import sb.faithd1ck.utils.MSTimer;
import sb.faithd1ck.utils.player.ItemUtils;
...
public class ModuleInvManager extends CheatModule {
    ...
    public final Handler<MotionEvent> onPreMotionEvent = event -> {
        ...
        if (!FaithD1ck.moduleManager.getModule(ModuleGuiMove.class).getState() && !(mc.currentScreen instanceof GuiInventory)) {
            return;
        }
        ...
        if (block != -1 && block != this.blockSlot.getValue().intValue() - 1 && !FaithD1ck.moduleManager.getModule(ModuleScaffold.class).getState()) {
    ...
    private boolean canOpenInventory() {
        return FaithD1ck.moduleManager.getModule(ModuleGuiMove.class).getState() && !(mc.currentScreen instanceof GuiInventory);
    }
