    ...
    public void execute(String[] args) {
        if (args.length > 1) {
            if ("load".equalsIgnoreCase(args[1])) {
                if (args.length > 2) {
                    ModuleConfig scriptFile = new ModuleConfig(new File(FaithD1ck.configManager.configsDir, args[2] + ".yml"));
            ...
            if ("save".equalsIgnoreCase(args[1])) {
                if (args.length > 2) {
                    ModuleConfig cfg = new ModuleConfig(new File(FaithD1ck.configManager.configsDir, args[2] + ".yml"));
                    try {
                        for (ModuleConfig config : FaithD1ck.configManager.configs) {
                            if (cfg.getFile().exists()) {
                                FaithD1ck.configManager.configs.remove(config);
                                config.getFile().delete();
                            }
                        }
                        saveConfig(cfg);
                        FaithD1ck.configManager.configs.add(cfg);
            ...
            if ("delete".equalsIgnoreCase(args[1])) {
                if (args.length > 2) {
                    ModuleConfig scriptFile = new ModuleConfig(new File(FaithD1ck.configManager.configsDir, args[2] + ".json"));
                    if (scriptFile.getFile().exists()) {
                        scriptFile.getFile().delete();
                        FaithD1ck.configManager.configs.removeIf(config -> scriptFile.getFile().getName().equals(config.getFile().getName()));
            ...
            if ("list".equalsIgnoreCase(args[1])) {
                File[] configs = FaithD1ck.configManager.configsDir.listFiles();
                if (configs != null) {
    ...
    public List<String> tabComplete(String[] args) {
        ...
        switch (args.length) {
            ...
            case 2:
                if (!"list".equalsIgnoreCase(args[1])) {
                    try {
                        if (!FaithD1ck.configManager.configs.isEmpty()) {
                            result.addAll(FaithD1ck.configManager.configs.stream().map(config -> config.getFile().getName().split(".json")[0]).filter(name -> name.startsWith(args[1])).collect(Collectors.toList()));
                        }
                    } catch (Exception e) {
                        if (!FaithD1ck.configManager.configs.isEmpty()) {
                            result.addAll(FaithD1ck.configManager.configs.stream().map(config -> config.getFile().getName().split(".json")[0]).collect(Collectors.toList()));
                        }
                    }
                }
                break;
