    ...
    protected void actionPerformed(GuiButton button) throws IOException
    {
        switch (button.id) {
            ...
            case 3:
                ...
                break;
            case 4:
                FaithD1ck.configManager.accountsConfig.removeAccount(altsList.getSelectedAccount());
                break;
            case 999:
                try {
                    ...
                    jf.dispose();
                    FaithD1ck.configManager.accountsConfig.addAccount(MicrosoftAccount.buildFromRefreshToken(token, MicrosoftAccount.AuthMethod.TOKENLOGIN));
                    FaithD1ck.configManager.accountsConfig.save();
                    FaithD1ck.notificationManager.pop("Alt Manager", "Done", NotificationType.SUCCESS);
                } catch (Exception e) {
                    FaithD1ck.notificationManager.pop("Alt Manager", "Failed login with token", NotificationType.ERROR);
                    e.printStackTrace();
                }
                break;
            case 998:
                try {
                    ...
                    if (returnVal == JFileChooser.APPROVE_OPTION) {
                        ...
                        if (!skinFile.getName().endsWith(".png")) {
                            FaithD1ck.notificationManager.pop("Its seems that the file isn't a skin..", NotificationType.ERROR);
                            break;
                        }
                        ...
                        if (response.getStatus() == 200 || response.getStatus() == 204) {
                            FaithD1ck.notificationManager.pop("Skin changed!", NotificationType.SUCCESS);
                        } else {
                            FaithD1ck.notificationManager.pop("Failed to change skin.", NotificationType.ERROR);
            ...
            case 997:
                try {
                    ...
                    HttpResponse response = HttpRequest.put(url)
                            ...
                            .execute();
                    if (response.getStatus() == 200 || response.getStatus() == 204) {
                        FaithD1ck.notificationManager.pop("Name changed!", NotificationType.SUCCESS);
                        ...
                        FaithD1ck.notificationManager.pop("Failed to change name due to " + cause, NotificationType.ERROR);
                        logger.error(response);
                    }
                } catch (Exception e) {
                    FaithD1ck.notificationManager.pop("Failed to change name.", NotificationType.ERROR);
                    e.printStackTrace();
                }
                break;
            case 996:
